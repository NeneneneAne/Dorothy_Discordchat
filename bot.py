...
def extract_gemini_text(response_json, default=""):
    ...
    return "".join(visible_texts).strip() or default


def contains_internal_text(text):
    """思考や内部指示らしい文をDiscordへ送らないための判定。"""
    markers = (
        "SYSTEM:",
        "USER:",
        "ASSISTANT:",
...
async def retry_without_thinking(url, headers, params, data):
    """内部文を拾った場合だけ、思考を止めて同じ内容を一度作り直す。"""
    retry_data = {
        **data,
        "generationConfig": {
            **data.get("generationConfig", {}),
            "thinkingConfig": {"thinkingBudget": 0}
        }
    }
    async with session.post(url, headers=headers, params=params, json=retry_data) as response:
        if response.status != 200:
            return ""
        return extract_gemini_text(await response.json(), "")
...
async def get_gemini_response(user_id, user_input):
    ...
    async with session.post(url, headers=headers, params=params, json=data) as response:
        ...
        if response.status == 200:
            ...
            # 通常は昔と同じ生成方法のまま。内部文が出たときだけ思考なしで再生成する
            if not reply_text or contains_internal_text(reply_text):
                reply_text = await retry_without_thinking(url, headers, params, data)

            if not reply_text or contains_internal_text(reply_text):
                reply_text = "えっ？うまくおへんじできなかったよ。もういちど話しかけてほしいな！"
...
async def get_gemini_response_with_image(user_id, user_input, image_bytes=None, image_mime_type="image/png"):
    ...
    async with session.post(url, headers=headers, params=params, json=data) as response:
        if response.status == 200:
            ...
            if not reply_text or contains_internal_text(reply_text):
                reply_text = "えっ？画像は見えたけど、うまくおへんじできなかったよ。もういちど送ってほしいな！"
