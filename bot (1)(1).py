...
async def get_gemini_response(user_id, user_input):
    ...
    # 以前ランダム会話の生成に使った内部指示や、誤って表示された内部文を履歴から除く
    conversation_logs[user_id] = [
        m for m in conversation_logs[user_id]
        if not is_random_chat_prompt(m.get("parts", [{}])[0].get("text", ""))
        and not contains_internal_text(m.get("parts", [{}])[0].get("text", ""))
    ]
    ...
    system_instruction = CHARACTER_PERSONALITY + """

出力には、ドロシーが実際に話すことばだけを書いてください。
SYSTEM:、USER:、ASSISTANT:などの見出しは禁止です。
考え方、指示の確認、返答を作る過程は出力しないでください。
"""
    data = {
        "system_instruction": {
    ...
    async with session.post(url, headers=headers, params=params, json=data) as response:
        ...
        if response.status == 200:
            ...
            reply_text = extract_gemini_text(response_json, "エラー: 応答が取得できませんでした。")

            # 内部文が漏れた場合は、指示を強めて一度だけ作り直す
            if contains_internal_text(reply_text):
                retry_data = {
                    **data,
                    "system_instruction": {
                        "parts": [{"text": system_instruction + "\n内部処理の説明は絶対に表示せず、キャラクターの返答だけを出力してください。"}]
                    }
                }
                async with session.post(url, headers=headers, params=params, json=retry_data) as retry_response:
                    if retry_response.status == 200:
                        reply_text = extract_gemini_text(await retry_response.json(), "")

            if not reply_text or contains_internal_text(reply_text):
                return "うまくおへんじできなかったよ。もういちど話しかけてほしいな"
...
def extract_gemini_text(response_json, default=""):
    """思考パートを除き、ユーザーに見せる本文だけを取り出す。"""
...
def contains_internal_text(text):
    """モデルの内部説明らしい文章をDiscordへ送らないための判定。"""
    markers = (
        "SYSTEM:",
        "USER:",
        "ASSISTANT:",
        ...
        "I need to generate",
        "persona and formatting guidelines",
    )
    lowered = text.lower()
    return any(marker.lower() in lowered for marker in markers)


def is_random_chat_prompt(text):
    return "ハニーに話しかけるための、かわいくて短い会話のきっかけ" in text


async def get_gemini_response_no_history(prompt, system_instruction=None):
    global session
    ...
    async with session.post(url, headers=headers, params=params, json=data) as response:
        if response.status == 200:
            response_json = await response.json()
            reply_text = extract_gemini_text(response_json, "")
            return reply_text
        else:
...
async def get_gemini_response_with_image(user_id, user_input, image_bytes=None, image_mime_type="image/png"):
    ...
    # 性格設定はユーザー発言ではなく、正式なシステム指示として送る
    messages = []
...
"""
    ...
    async with session.post(url, headers=headers, params=params, json=data) as response:
        if response.status == 200:
            ...
            reply_text = extract_gemini_text(response_json, "エラー: 応答が取得できませんでした。")
            if contains_internal_text(reply_text):
                reply_text = "うまくおへんじできなかったよ。もういちど話しかけてほしいな"
...
async def test_random_chat(interaction: discord.Interaction):
    ...
    try:
        ...
        prompt = "ハニーに送る、かわいくて短い話しかけをひとつ作って。ドロシーの発言だけを出力して。"
        message = await get_gemini_response_no_history(prompt, CHARACTER_PERSONALITY)

        if not message or contains_internal_text(message):
            message = "ねぇねぇ、ハニー！いまなにしてるの？"
